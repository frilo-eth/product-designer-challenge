 ...
 *
 * Proxies to: GET /{chainId}/{vaultAddress}/live/liquidity-profile?refresh=false
  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        {
          error: 'Failed to fetch liquidity profile',
    ...
    console.error('Error fetching liquidity profile:', error)
    return NextResponse.json(
      {
