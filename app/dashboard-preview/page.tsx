...
export default function DashboardPreviewPage() {
  ...
  // Simple initial loading state - true until first data arrives
  ...
  // Handle disconnect (for now just visual, log to console)
  ...
  const loadPriceImpact = useCallback(
    async (
      ...
      try {
        ...
        setHasInitialData(true) // Even on error, we have "initial data"
        setPriceImpactError(
  ...
  useEffect(() => {
    ...
    // Reset hasInitialData when switching vaults (but only if actually switching)
    if (previousVaultRef.current !== selectedVaultKey) {
      // Check if we have cached data - if so, don't reset (instant load)
      const hasCachedData = vaultDataCache.get(selectedVault.chainId, selectedVault.address, 'vaultDetails')
      if (!hasCachedData) {
